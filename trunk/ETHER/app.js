...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('identification', function(participant, mdpEntre){
    if(validation(participant, mdpEntre)){
      ...
      socket.emit('identification echouee');
      console.log(
  ...
  socket.on('reception client', function(m, cle_emetteur, succes){
    io.sockets.socket(participants[cle_emetteur].socketID).emit('reception serveur', m, maCle, succes);
  });
});
